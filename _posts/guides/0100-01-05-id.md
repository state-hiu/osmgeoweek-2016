...
* We make use of the ID Editor in this Guide (although tagging concepts apply to the JOSM editor, as well). More advanced users may want to migrate to the [JOSM editor]({{site.baseurl}}/guides/josm-editor) at some point for improved editing efficiency or specialized needs.
* In ID Editor, shortcuts are accessed by first clicking on the feature (which will be either a line or a point). This brings up several options to edit the feature. Available shortcuts are specified below each editing option on the editing wheel. Learning shortcuts can save time!
